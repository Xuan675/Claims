...
AMT_TOKEN = re.compile(
    rf"(?:{CURRENCY}\s*(?:{DECIMAL}|{INTEGER})|(?<!\d){DECIMAL}\s*{CURRENCY})(?!\d)",
    re.I
)
...
def _extract_ragged_text_table(block: str, page1: int, fallback_date: str, page_currency: str) -> list[LineItem]:
    ...
    for i, line in enumerate(lines):
        ...
        is_total_row = looks_like_total(left)
        if (SKIP_WORDS.search(left) and not is_total_row) or TABLE_HEADER_RE.search(left):
            continue
        ...
        if _nonzero(amount) and desc and len([t for t in _desc_tokens(desc)]) >= 2:
            src = "text_row_total" if is_total_row else "text_row"
        ...
        for docno, gdoc in df.groupby("docno", sort=False):
            ...
            for _, row in gdoc.iterrows():
                ...
                if cand_amt_ok.empty:
                    ...
                    is_total_pdf = ("total" in str(row["source"]).lower()) or bool(TOTAL_WORDS.search(str(row["description"])))
                    if is_total_pdf:
                        # sum all Excel rows for this docno (the same 'pool' you already built)
                        book_sum = float(pool["Amount"].fillna(0).sum()) if not pool.empty else 0.0
                        ...
                        # Compare in the same currency (your claim book is SGD, and your extracted totals are SGD in the examples)
                        if abs(book_sum - amount) <= tol_abs_eff:
                            # We treat this as a *doc-level* OK. Do NOT consume specific rows.
                            matches.append({
                                **row.drop(labels=["_ndesc"]).to_dict(),
                                "actual_row": None,
                            ...
                            continue  # done with this PDF total row
